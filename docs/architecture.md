...
- **Node & Memory Safety:** Each `Tensor` generated by an operation is accompanied by a `Node` storing that operation and its parent `Tensor`s. `AutogradMeta` memory ownership is governed by `std::shared_ptr`, and `AccumulateGrad` explicitly uses `std::weak_ptr` to avoid circular references and gracefully prevent Use-After-Free (UAF) corruption during graph teardown.
...
- **AutoTuner Profiler:** A JIT profiler that measures hardware capabilities via Lazy Evaluation or Explicit Initialization to determine the optimal FLOP threshold for threading.
- **CPU Backend Dispatcher:** Acts as a traffic controller, checking configurations (`MatMulConfig`, `AVX2 Support`, `AutoTuner` cache) and choosing the most appropriate Backend.
- **Backend Implementations:** Provides MatMul (Matrix Multiplication) and Element-wise vector computations optimized for various scenarios:
  ...
  - `AVX2 (SIMD)`: Hardware instruction level optimization with `4x16` Outer Product micro-kernels utilizing 256-bit YMM registers.
